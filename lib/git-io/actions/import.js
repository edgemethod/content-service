...
module.exports = function (repoName, r) {
  ...
  try {
    r.db(process.env.CACHE_DB_NAME).table(repoName).delete().run();
  } catch(e) {
    console.log(`${repoName} table removal error: ${e}`)
  }

  try {
    r.db(process.env.CACHE_DB_NAME).tableCreate(repoName);
  } catch(e) {
    console.log(`${repoName} table creation error: ${e}`)
  }
  
  const db = r.db(process.env.CACHE_DB_NAME).table(repoName);
